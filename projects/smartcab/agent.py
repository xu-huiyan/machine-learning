...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False, epsilon=1.0, alpha=0.5, trail_num=1):
    ...
    def choose_action(self, state):
        ...
        if self.learning==False:
            ...
            if random.random()<self.epsilon:
                ...
                print(maxQ)
                Q_sub=self.Q[state]
                #maxQ appears more than once, select random among maxQ actions
                if list(Q_sub.values()).count(maxQ)>1:
                    temp_action_list=[]
                    for k in Q_sub.keys():
                        if Q_sub[k]==maxQ:
                            temp_action_list.append(k)
                    action=random.choice(temp_action_list)
                #maxQ appears once, return the action
                else:
                    for k in Q_sub.keys():
                        if Q_sub[k]==maxQ:
                            action=k
